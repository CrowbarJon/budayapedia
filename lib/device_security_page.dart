...
class _DeviceSecurityPageState extends State<DeviceSecurityPage> {
  ...
  static List<Map<String, dynamic>> activeSessions = [
    {
      'device': 'Samsung Galaxy S23 (Perangkat Ini)',
  ...
  static List<Map<String, dynamic>> activityLog = [
    {
      'activity': 'Ubah Kata Sandi Berhasil',
      'time': '20 Des, 14:30',
      'isAlert': false,
    },
    {
      'activity': 'Login di Perangkat Baru (iPhone 14 Pro)',
      'time': '19 Des, 10:00',
      'isAlert': true,
    },
    {
      'activity': 'Akses Riwayat Keamanan',
      'time': '18 Des, 08:00',
      'isAlert': false,
    },
    {
      'activity': 'Login Berhasil (Samsung S23)',
      'time': '17 Des, 19:00',
      'isAlert': false,
    },
  ];
  ...
  void _logoutDevice(int index) {
    if (activeSessions[index]['isCurrent']) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
          content: Text(
            "Tidak bisa mengeluarkan perangkat yang sedang Anda gunakan.",
          ),
        ),
      );
      return;
    ...
    showDialog(
      ...
      builder: (context) => AlertDialog(
        ...
        content: Text(
          "Yakin ingin mengeluarkan sesi dari ${activeSessions[index]['device']}?",
        ),
        actions: [
          TextButton(
          ...
          ElevatedButton(
            onPressed: () {
              ...
              ScaffoldMessenger.of(context).showSnackBar(
                SnackBar(
                  content: Text("$loggedOutDevice berhasil dikeluarkan!"),
                ),
              );
            },
  ...
  Widget _buildActivityLogItem(Map<String, dynamic> log) {
    ...
    return ListTile(
      leading: Icon(
        log['isAlert'] ? Icons.notification_important : Icons.check_circle,
        color: color,
      ),
      title: Text(
        log['activity'] as String,
